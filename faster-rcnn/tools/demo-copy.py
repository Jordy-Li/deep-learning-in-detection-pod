...
def vis_detections(image_name, im, class_name, dets, thresh=0.5):    # 此处的函数添加一个形参
    ...
    plt.draw()
    # 添加下面两行，注意修改路径
    plt.savefig('/media/zhengda/共享文件存档/tf-faster-rcnn-master/result/'+image_name)   # 保存结果的路径
    print("save image to /media/zhengda/共享文件存档/tf-faster-rcnn-master/result/{}".format(image_name))
...
if __name__ == '__main__':
    ...
    # 添加下面几行
    fi = open('/media/zhengda/共享文件存档/tf-faster-rcnn-master/data/VOCdevkit2007/VOC2007/ImageSets/Main/test.txt')
	#输入要批量检测的图片名字合集，直接用训练时的test.txt就行。
    ...
    for im_name in im_names:
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        print('Demo for data/demo-copy/{}'.format(im_name))
